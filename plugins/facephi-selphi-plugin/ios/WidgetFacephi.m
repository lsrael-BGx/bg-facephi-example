...
@interface WidgetFacephi : CDVPlugin<FPhiWidgetProtocol>
    ...
    - (void)GenerateTemplateRaw:(CDVInvokedUrlCommand*)command;
...
- (void)StartWidget:(CDVInvokedUrlCommand*)command
    {
        ...
        if (config != nil) {
            ...
            self.uc = [[SelphiWidget alloc]initWithFrontCameraIfAvailable :((NSNumber *)isCameraFrontalPreferred).boolValue  resources:strBundle delegate:self error:&error];
            else
            self.uc = [[SelphiWidget alloc]initWithFrontCameraIfAvailable :true  resources:strBundle delegate:self error:&error];
        } else {
            self.uc = [[SelphiWidget alloc]initWithFrontCameraIfAvailable :true  resources:strBundle delegate:self error:&error];
        }
        if (error != nil) {
        ...
        if (config != nil) {
            ...
            if (livenessMode != nil && livenessMode != (NSString*)[NSNull null]){
                ...
                else if ([livenessMode compare:@"PASSIVE" ] == NSOrderedSame)
    ...
    {
        @try {
            NSLog(@"GenerateTemplateRaw method fired.");
            NSDictionary* arguments = [command.arguments objectAtIndex:0];
            NSString *imageBase64 = [arguments objectForKey:@"imageBase64"];
                
            UIImage* rawImage = [WidgetFacephi base64toUIImage:imageBase64];
            NSData *imageTemplate = [FPhiWidget generateTemplateRawFromUIImage:rawImage];
            NSString* imageTemplateBase64 = [WidgetFacephi imageToBase64:imageTemplate];

            CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:imageTemplateBase64];
...
-(void) ExtractionFinished {
    ...
    if(self.uc.results.bestImage != nil) {
            ...
            NSData *data = UIImageJPEGRepresentation([self scaleAndRotateImage:record.image], 0.95f);
            NSString *base64String = [WidgetFacephi base64forData:data];
...
+ (UIImage*)base64toUIImage:(NSString*)imageString {
    ...
    return [UIImage imageWithData:str];
}
