...
- (void)StartCapture:(CDVInvokedUrlCommand*)command
{
    ...
    // MyPluginClass* pluginInstance = [self.viewController getCommandInstance:@"ParsePushPlugin"];
    NSString* googleCred = [bundle objectForInfoDictionaryKey:@"GoogleAppID"];
    NSString* senderCred = [bundle objectForInfoDictionaryKey:@"GCMSenderID"];
    
    if(senderCred == nil || googleCred == nil || [senderCred compare:@"default-value"] == NSOrderedSame || [googleCred compare:@"default-value"] == NSOrderedSame) {
        _uc = [[FPhiSelphIDWidget alloc]init :true
                                             :[bundle pathForResource:resourcesPath ofType:@"zip"]
                                             :self
                                  withLicense:license
                                             :&error];
    }
    else {
        NSDictionary* firCredentials = @{@"GoogleAppID":googleCred, @"GCMSenderID":senderCred};
